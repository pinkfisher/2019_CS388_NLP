...
def _parse_args():
    ...
    parser.add_argument('--debug', dest='debug', default=False, action='store_true', help='debug the model as autoencoder')
    parser.add_argument('--bidirect', dest='bidirect', default=False, action='store_true', help='use bidirect encoder')
...
def train_model_encdec(train_data: List[Example], test_data: List[Example], input_indexer, output_indexer, args) -> Seq2SeqSemanticParser:
    ...
    len_sentence = all_train_input_data.shape[1]

    input_emb = EmbeddingLayer(len_sentence, enc_input_size, 0)

    if args.bidirect:
        encoder = RNNEncoder(enc_input_size, enc_hidden_size, bidirect=True)
    else:
    ...
    for epoch in range(1, n_epochs+1):
        ...
        for i in ex_indices:
            input_tensor = torch.from_numpy(all_train_input_data[i, :])
            target_tensor = torch.from_numpy(all_train_output_data[i, :])
            input_lens_tensor = torch.tensor([input_lens[i]])
            print(input_lens_tensor)
            if args.debug:
              target_tensor = input_tensor
            if args.bidirect:
                loss = train_model(input_tensor.unsqueeze(0), target_tensor.unsqueeze(0), encoder, decoder, encoder_optimizer, decoder_optimizer,
                               criterion, output_indexer, input_emb=input_emb, input_lens_tensor=input_lens_tensor)
            else:
                loss = train_model(input_tensor.unsqueeze(1), target_tensor.unsqueeze(1), encoder, decoder, encoder_optimizer, decoder_optimizer,
...
def train_model(input_tensor, target_tensor, encoder, decoder, encoder_optimizer, decoder_optimizer,
                criterion, output_indexer, input_emb=None, input_lens_tensor=None, max_length=65, teacher_forcing_ratio=0.5):
    ...
    if not args.bidirect:
        ...
        input_emb = input_emb.forward(input_tensor)
        (encoder_output, enc_context_mask, encoder_hidden) = encoder.forward(input_emb, input_lens_tensor)

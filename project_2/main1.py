...
def train_model_encdec(train_data: List[Example], test_data: List[Example], input_indexer, output_indexer, args) -> Seq2SeqSemanticParser:
    ...
    model_input_emb = EmbeddingLayer(enc_input_size, len(input_indexer), 0)
    ...
    for epoch in range(n_epochs):
        for i in range(n_sentences):
            ...
            input_len = torch.from_numpy(np.array([input_lens[i]]))
            target_tensor = torch.from_numpy(all_train_output_data[i, :]).unsqueeze(0)
            target_len = torch.from_numpy(np.array([output_lens[i]]))
            loss = train_model(input_tensor, target_tensor, input_len, target_len, model_input_emb, encoder, decoder, encoder_optimizer, decoder_optimizer, criterion)
            print_loss_total += loss
            plot_loss_total += loss


def train_model(input_tensor, target_tensor, input_len, target_len, model_input_emb, encoder, decoder, encoder_optimizer, decoder_optimizer,
          criterion, max_length=65, teacher_forcing_ratio=1):

    encoder_optimizer.zero_grad()
    decoder_optimizer.zero_grad()

    # Run through encoder
    input_emb = model_input_emb.forward(input_tensor)
    encoder_outputs, enc_context_mask, enc_final_states = encoder.forward(input_emb, input_len)
    encoder_hidden = (enc_final_states[0].unsqueeze(0), enc_final_states[1].unsqueeze(0))

    decoder_input_tensor = torch.tensor([[1]])
    decoder_input = model_input_emb.forward(decoder_input_tensor)
